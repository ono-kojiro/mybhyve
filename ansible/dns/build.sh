...
origfiles="db.example.com.orig db.192.168.0.orig"
for origfile in $origfiles; do
  if [ -e "$origfile" ]; then
    src="$origfile"
    dst=`basename $origfile .orig`
    cp -f $src $dst
  fi
done
...
all()
{
  ansible-playbook -i hosts.yml -t all site.yml
}

deploy()
{
  ansible-playbook -i hosts.yml -t dns site.yml
}
